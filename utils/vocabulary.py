...
class Vocab(object):
    ...
    def write_to_file(self, vocab_file):
    ...
    def convert_to_tensor(self, symbols, format='torch', type='int64'):

        # TODO: add option to convert to numpy first (for multi-threading serialization)
        vector = self.get_indices(symbols)
        if format == 'torch':
            if type in ['int64', 'long']:
                return torch.LongTensor(vector)
            elif type in ['int32', 'int']:
                return torch.IntTensor(vector)
            elif type in ['int16', 'short']:
                return torch.ShortTensor(vector)
            ...
            if type in ['int64', 'long']:
                ...
                return np.array(vector, dtype=np.int16)
